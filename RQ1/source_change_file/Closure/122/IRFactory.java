...
class IRFactory {
  ...
  private IRFactory(String sourceString, StaticSourceFile sourceFile, Config config, ErrorReporter errorReporter) {
    ...
    switch (config.languageMode) {
      case ECMASCRIPT3: reservedKeywords = null; // use TokenStream.isKeyword instead break;
  ...
  private void validateTypeAnnotations(JSDocInfo info, AstNode node) {
    if (info.hasType()) {
      ...
      switch (node.getType()) {
        ...
        case com.google.javascript.rhino.head.Token.NAME: AstNode parent = node.getParent();
        ...
        case com.google.javascript.rhino.head.Token.CALL: valid = info.isDefine();
          break;
      }
      if (!valid) {
  ...
  private Node maybeInjectCastNode(AstNode node, JSDocInfo info, Node irNode) {
    if (node.getType() == com.google.javascript.rhino.head.Token.LP && node instanceof ParenthesizedExpression && info.hasType()) {
  ...
  private JSDocInfo parseInlineTypeDoc(Comment node, Node irNode) {
    ...
    // The JsDocInfoParser expects the comment without the initial '/**'.
    int numOpeningChars = 3;
    JsDocInfoParser parser = new JsDocInfoParser( new JsDocTokenStream(comment.substring(numOpeningChars), lineno, position2charno(position) + numOpeningChars), node, irNode, config, errorReporter);
   ...
   Node getFnParamNode(Node fnNode) {
    ...
    private boolean validAssignmentTarget(Node target) {
      switch (target.getType()) {
        case Token.CAST: // CAST is a bit weird, but syntactically valid. case Token.NAME: case Token.GETPROP: case Token.GETELEM: return true;
      }
      return false;
    }
