...
final class ControlFlowAnalysis implements Callback, CompilerPass {
   ...
   *    by default).
   */
  ControlFlowAnalysis(AbstractCompiler compiler, boolean shouldTraverseFunctions) {
  ...
  @Override public void visit(NodeTraversal t, Node n, Node parent) {
    switch (n.getType()) {
      ...
      case Token.CASE: handleCase(n);
        return;
      case Token.DEFAULT: handleDefault(n);
        return;
  ...
  private void handleIf(Node node) {
    ...
    if (elseBlock == null) {
      createEdge(node, Branch.ON_FALSE, computeFollowNode(node)); // not taken branch } else {
  ...
  private void handleSwitch(Node node) {
    ...
    if (next != null) { // Has at least one CASE or EMPTY createEdge(node, Branch.UNCOND, next);
    } else { // Has no CASE but possibly a DEFAULT if (node.getFirstChild().getNext() != null) {
        createEdge(node, Branch.UNCOND, node.getFirstChild().getNext());
      } else { // No CASE, no DEFAULT createEdge(node, Branch.UNCOND, computeFollowNode(node));
      }
    }
  ...
  private void handleCase(Node node) {
    ...
    if (next != null) { // Found a CASE Preconditions.checkState(next.getType() == Token.CASE);
      createEdge(node, Branch.ON_FALSE, next);
    } else { // No more CASE found, go back and search for a DEFAULT. Node parent = node.getParent();
      Node deflt = getNextSiblingOfType( parent.getFirstChild().getNext(), Token.DEFAULT);
      if (deflt != null) { // Has a DEFAULT createEdge(node, Branch.ON_FALSE, deflt);
      } else { // No DEFAULT found, go to the follow of the SWITCH. createEdge(node, Branch.ON_FALSE, computeFollowNode(node));
      }
    }
    connectToPossibleExceptionHandler(node, node.getFirstChild());
  }
  ...
  private void handleStmtList(Node node) {
    ...
    if (parent != null) {
      switch (parent.getType()) {
        case Token.DEFAULT: case Token.CASE: case Token.TRY: break;
        default: if (node.getType() == Token.BLOCK && node.isSyntheticBlock()) {
  ...
  private Node computeFollowNode(Node fromNode, Node node) {
    ...
    switch (parent.getType()) {
      ...
      case Token.IF: return computeFollowNode(fromNode, parent);
      ...
      case Token.TRY:
        ...
        if (parent.getFirstChild() == node) {
          if (NodeUtil.hasFinally(parent)) { // and have FINALLY block. return computeFallThrough(parent.getLastChild());
          } else { // and have no FINALLY. return computeFollowNode(fromNode, parent);
          }
        // CATCH block.
        } else if (NodeUtil.getCatchBlock(parent) == node){
          if (NodeUtil.hasFinally(parent)) { // and have FINALLY block. return computeFallThrough(node.getNext());
          } else {
            return computeFollowNode(fromNode, parent);
          }
  ...
  private void connectToPossibleExceptionHandler(Node cfgNode, Node target) {
    if (mayThrowException(target) && !exceptionHandler.isEmpty()) {
      ...
      for (Node handler : exceptionHandler) {
        ...
        if (!NodeUtil.hasCatchHandler(catchBlock)) { // No catch but a FINALLY. if (lastJump == cfgNode) {
            createEdge(cfgNode, Branch.ON_EX, handler.getLastChild());
          } else {
            finallyMap.put(lastJump, handler.getLastChild());
          }
        } else { // Has a catch. if (lastJump == cfgNode) {
  ...
  private static boolean mayThrowException(Node n) {
    switch (n.getType()) {
      case Token.CALL: case Token.GETPROP: case Token.GETELEM: case Token.THROW: case Token.NEW: case Token.ASSIGN: case Token.INC: case Token.DEC: return true;
  ...
  private static class AstControlFlowGraph extends ControlFlowGraph<Node> {
     ...
     */
    private AstControlFlowGraph(Node entry, Map<DiGraphNode<Node, Branch>, Integer> priorities) {
      super(entry);
