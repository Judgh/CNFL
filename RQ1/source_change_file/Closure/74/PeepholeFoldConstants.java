...
class PeepholeFoldConstants extends AbstractPeepholeOptimization {
  ...
  @Override Node optimizeSubtree(Node subtree) {
    switch(subtree.getType()) {
      case Token.CALL: return tryFoldKnownMethods(subtree);
    ...
    if (left.getType() == Token.NUMBER && right.getType() == Token.NUMBER) {
      ...
      switch (n.getType()) {
        ...
        case Token.URSH:
          ...
          break;
        default: throw new AssertionError("Unknown shift operator: " + Node.tokenToName(n.getType()));
      }
  ...
  @SuppressWarnings("fallthrough") private Node tryFoldComparison(Node n, Node left, Node right) {
    ...
    switch (lhType) {
      case Token.VOID: if (!NodeUtil.isLiteralValue(left.getFirstChild(), false)) {
          return n;
      ...
      case Token.NULL: case Token.TRUE: case Token.FALSE: if (undefinedRight) {
      ...
      case Token.THIS: if (right.getType() != Token.THIS) {
          return n;
        }
        switch (op) {
      ...
      case Token.STRING: if (undefinedRight) {
        ...
        switch (op) {
          case Token.SHEQ: case Token.EQ: result = left.getString().equals(right.getString());
            break;

          case Token.SHNE: case Token.NE: result = !left.getString().equals(right.getString());
            break;
      ...
      case Token.NAME: if (undefinedRight) {
          result = compareToUndefined(left, op);
          break;
        }
  ...
  private Boolean compareAsNumbers(int op, Node left, Node right) {
    ...
    Boolean result;
    switch (op) {
      case Token.SHEQ: case Token.EQ: Preconditions.checkState( left.getType() == Token.NUMBER && right.getType() == Token.NUMBER);
        result = lv == rv;
        break;
      case Token.SHNE: case Token.NE: Preconditions.checkState( left.getType() == Token.NUMBER && right.getType() == Token.NUMBER);
        result = lv != rv;
        break;
  ...
  private boolean inForcedStringContext(Node n) {
    return n.getParent().getType() == Token.GETELEM && n.getParent().getLastChild() == n;
  }
  ...
  private Node tryFoldArrayJoin(Node n) {
    ...
    switch (arrayFoldedChildren.size()) {
      case 0: Node emptyStringNode = Node.newString("");
        n.getParent().replaceChild(n, emptyStringNode);
        reportCodeChange();
        return emptyStringNode;
      case 1: Node foldedStringNode = arrayFoldedChildren.remove(0);
  ...
  private Node tryFoldObjectPropAccess(Node n, Node left, Node right) {
    ...
    Node parent = n.getParent();
    if ((NodeUtil.isAssignmentOp(parent) && parent.getFirstChild() == n) || parent.getType() == Token.INC || parent.getType() == Token.DEC) {
    ...
    for (Node c = left.getFirstChild(); c != null; c = c.getNext()) {
      if (c.getString().equals(right.getString())) {
        switch (c.getType()) {
          case Token.SET: continue;
          case Token.GET: case Token.STRING: if (value != null && mayHaveSideEffects(value)) {
