...
class FunctionInjector {
  ...
  private enum CallSiteType {
     ...
     */
    UNSUPPORTED() {
      @Override public void prepare(FunctionInjector injector, Node callNode) {
  ...
  private Node inlineFunction( Node callNode, Node fnNode, String fnName) {
    ...
    switch (callSiteType) {
      case SIMPLE_ASSIGNMENT: resultName = parent.getFirstChild().getString();
        break;

      case VAR_DECL_SIMPLE_ASSIGNMENT: resultName = parent.getString();
        break;

      case SIMPLE_CALL: resultName = null;  // "foo()" doesn't need a result. needsDefaultReturnResult = false;
        break;

      case EXPRESSION: throw new IllegalStateException( "Movable expressions must be moved before inlining.");

      case DECOMPOSABLE_EXPRESSION: throw new IllegalStateException( "Decomposable expressions must be decomposed before inlining.");
  ...
  private static int inlineCostDelta( Node fnNode, Set<String> namesToAlias, InliningMode mode) {
    ...
    if (mode == InliningMode.DIRECT) {
      ...
      final int inlineBlockOverhead = 4; // "X:{}" final int perReturnOverhead = 2;   // "return" --> "break X" final int perReturnResultOverhead = 3; // "XX=" final int perAliasOverhead = 3; // "XX="
      final int perReturnOverhead = 2;   // "return" --> "break X" final int perReturnResultOverhead = 3; // "XX=" final int perAliasOverhead = 3; // "XX="
      final int perReturnResultOverhead = 3; // "XX=" final int perAliasOverhead = 3; // "XX="
