        ...
        case ANNOTATION: if (state == State.SEARCHING_ANNOTATION) {
            ...
            if (annotation == null) {
              ...
              switch (annotation) {
                ...
                case DEFINE: case RETURN: case THIS: case TYPE: case TYPEDEF: lineno = stream.getLineno();
                  charno = stream.getCharno();
                  ...
                  if (type == null) {
                    ...
                    switch (annotation) {
                      ...
                      case RETURN: if (!jsdocBuilder.recordReturnType(type)) {
                          parser.addTypeWarning( "msg.jsdoc.incompat.type", lineno, charno);
                          break;
                        }
                      ...
                      case TYPE: if (!jsdocBuilder.recordType(type)) {
                          parser.addTypeWarning( "msg.jsdoc.incompat.type", lineno, charno);
                        }
                        break;
  ...
  @SuppressWarnings("fallthrough") private ExtractionInfo extractMultilineTextualBlock(JsDocToken token, WhitespaceOption option) {
    ...
    do {
      switch (token) {
        ...
        case ANNOTATION: case EOC: case EOF:
        case EOC: case EOF:
        case EOF:
