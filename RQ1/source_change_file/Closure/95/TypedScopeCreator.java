...
final class TypedScopeCreator implements ScopeCreator {
    ...
    @Override public void visit(NodeTraversal t, Node node, Node parent) {
      ...
      switch (node.getType()) {
        case Token.VAR: for (Node child = node.getFirstChild();
          ...
          break;
        case Token.EXPR_RESULT: Node maybeAssign = node.getFirstChild();
  ...
  private void attachLiteralTypes(Node n) {
    switch (n.getType()) {
      ...
      case Token.REF_SPECIAL: n.setJSType(getNativeType(UNKNOWN_TYPE));
        break;

      case Token.OBJECTLIT: if (n.getJSType() == null) {
  ...
  private abstract class AbstractScopeBuilder implements NodeTraversal.Callback {
    ...
    @Override public void visit(NodeTraversal t, Node n, Node parent) {
      ...
      switch (n.getType()) {
        ...
        case Token.CATCH: defineCatch(n, parent);
          break;

        case Token.VAR: defineVar(n, parent);
          break;
     ...
     */
    JSType getDeclaredTypeInAnnotation( NodeTraversal t, Node node, JSDocInfo info) {
    ...
    private FunctionType getFunctionType(String name, Node rValue, JSDocInfo info, @Nullable Node lvalueNode) {
      ...
      if (functionType == null) {
        ...
        functionType = new FunctionTypeBuilder(name, compiler, errorRoot, sourceName, scope) .setSourceNode(fnRoot) .inferFromOverriddenFunction(overriddenPropType, parametersNode) .inferTemplateTypeName(info) .inferReturnType(info) .inferInheritance(info) .inferThisType(info, owner) .inferParameterTypes(parametersNode, info) .inferReturnStatementsAsLastResort(fnBlock) .buildAndRegister();
      }
        ...
        if (n.getType() == Token.EXPR_RESULT) {
          ...
          switch (child.getType()) {
            case Token.ASSIGN: maybeCollectMember(t, child.getFirstChild(), child);
              break;
            case Token.GETPROP: maybeCollectMember(t, child, child);
              break;
          }
        }
      }

      private void maybeCollectMember(NodeTraversal t, Node member, Node nodeWithJsDocInfo) {
