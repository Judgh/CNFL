...
public class PeepholeFoldConstants extends AbstractPeepholeOptimization {
  ...
  @Override Node optimizeSubtree(Node subtree) {
    switch(subtree.getType()) {
      ...
      case Token.NOT: case Token.NEG: case Token.BITNOT: return tryFoldUnaryOperator(subtree);

      default: return tryFoldBinaryOperator(subtree);
    }
  }
  ...
  private Node tryFoldBinaryOperator(Node subtree) {
    ...
    switch(subtree.getType()) {
      ...
      case Token.BITAND: case Token.BITOR: return tryFoldBitAndOr(subtree, left, right);
      ...
      case Token.SUB: case Token.MUL: case Token.DIV: return tryFoldArithmetic(subtree, left, right);
  ...
  private Node tryFoldUnaryOperator(Node n) {
    ...
    switch (n.getType()) {
      case Token.NOT: int result = leftVal.toBoolean(true) ? Token.FALSE : Token.TRUE;
  ...
  private Node tryFoldBitAndOr(Node n, Node left, Node right) {
    ...
    if (left.getType() == Token.NUMBER && right.getType() == Token.NUMBER) {
      ...
      switch (n.getType()) {
        case Token.BITAND: resultDouble = lvalInt & rvalInt;
          break;
        case Token.BITOR: resultDouble = lvalInt | rvalInt;
          break;
  ...
  @SuppressWarnings("fallthrough") private Node tryFoldComparison(Node n, Node left, Node right) {
    ...
    switch (left.getType()) {
      ...
      case Token.NUMBER: if (undefinedRight) {
        ...
        switch (op) {
          case Token.SHEQ: case Token.EQ: result = lv == rv; break;
          case Token.SHNE: case Token.NE: result = lv != rv; break;
