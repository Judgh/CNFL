...
class CodeGenerator {
  ...
  void add(Node n, Context context) {
    ...
    switch (type) {
      ...
      case Token.NAME: if (first == null || first.isEmpty()) {
          addIdentifier(n.getString());
        } else {
      ...
      case Token.NUMBER: Preconditions.checkState(childCount == 0);
      ...
      case Token.GETTER_DEF: case Token.SETTER_DEF: Preconditions.checkState(n.getParent().isObjectLit());
        Preconditions.checkState(childCount == 1);
      ...
      case Token.NULL: Preconditions.checkState(childCount == 0);
        cc.addConstant("null");
        break;

      case Token.THIS: Preconditions.checkState(childCount == 0);
        add("this");
        break;

      case Token.FALSE: Preconditions.checkState(childCount == 0);
        cc.addConstant("false");
        break;

      case Token.TRUE: Preconditions.checkState(childCount == 0);
        cc.addConstant("true");
        break;
      ...
      case Token.STRING_KEY: Preconditions.checkState( childCount == 1, "Object lit key must have 1 child");
        addJsString(n);
        break;

      case Token.STRING: Preconditions.checkState( childCount == 0, "A string may not have children");
        addJsString(n);
        break;
      ...
      case Token.OBJECTLIT: {
        ...
        for (Node c = first; c != null; c = c.getNext()) {
          ...
          if (c.isGetterDef() || c.isSetterDef()) {
            ...
            if (!c.isQuotedString() && !(languageMode == LanguageMode.ECMASCRIPT3 && TokenStream.isKeyword(key)) && TokenStream.isJSIdentifier(key)
                && !(languageMode == LanguageMode.ECMASCRIPT3 && TokenStream.isKeyword(key)) && TokenStream.isJSIdentifier(key)
                    && TokenStream.isKeyword(key)) && TokenStream.isJSIdentifier(key)
      ...
      case Token.DEFAULT_CASE: Preconditions.checkState(childCount == 1);
        add("default");
        addCaseBody(first);
        break;
  ...
  private String strEscape( String s, char quote, String doublequoteEscape, String singlequoteEscape, String backslashEscape, CharsetEncoder outputCharsetEncoder, boolean useSlashV, boolean isRegexp) {
    ...
    for (int i = 0; i < s.length(); i++) {
      ...
      switch (c) {
        ...
        case '&': if (trustedStrings || isRegexp) {
            sb.append(c);
          } else {
            sb.append("\\x26");
          }
