...
*
 */
class FoldConstants extends AbstractPostOrderCallback implements CompilerPass {
  ...
  public void visit(NodeTraversal t, Node n, Node parent) {
    ...
    if (type == Token.TYPEOF && NodeUtil.isLiteralValue(left)) {
      ...
      switch (left.getType()) {
        case Token.STRING: newValue = "string";
          break;
        case Token.NUMBER: newValue = "number";
          break;
        case Token.TRUE: case Token.FALSE: newValue = "boolean";
          break;
        case Token.NULL: case Token.OBJECTLIT: case Token.ARRAYLIT: newValue = "object";
          break;
        case Token.NAME:
    ...
    if (type == Token.NOT || type == Token.NEG || type == Token.BITNOT) {
        ...
        switch (type) {
          case Token.NOT: int result = NodeUtil.getBooleanValue(left) ? Token.FALSE : Token.TRUE;
    ...
    while (true) {
      switch (n.getType()) {
        case Token.IF: if (n.getChildCount() < 3) return true;
          ...
          n = n.getLastChild();
          continue;
        case Token.WITH: case Token.WHILE: case Token.FOR: n = n.getLastChild();
          continue;
  ...
  void tryFoldBlock(NodeTraversal t, Node n, Node parent) {
    ...
    for (Node c = n.getFirstChild(); c != null; ) {
      Node next = c.getNext();  // save c.next, since 'c' may be removed if (!NodeUtil.mayHaveSideEffects(c)) {
        n.removeChild(c);  // lazy kids t.getCompiler().reportCodeChange();
      }
      c = next;
    }
  ...
  private boolean tryMinimizeNot(NodeTraversal t, Node n, Node parent) {
    ...
    switch (notChild.getType()) {
      case Token.EQ: complementOperator = Token.NE;
        break;
      case Token.NE: complementOperator = Token.EQ;
        break;
      case Token.SHEQ: complementOperator = Token.SHNE;
        break;
      case Token.SHNE: complementOperator = Token.SHEQ;
        break;
  ...
  private void tryRemoveRepeatedStatements(NodeTraversal t, Node n) {
    ...
    while (true) {
      ...
      if (lastTrue == null || lastFalse == null || !compiler.areNodesEqualForInlining(lastTrue, lastFalse)) {
        break;
      }
  ...
  private void tryReduceReturn(NodeTraversal t, Node n) {
    ...
    if (result != null) {
      switch (result.getType()) {
        case Token.VOID: Node operand = result.getFirstChild();
   ...
   * the argument?
   */
  private boolean isLowerPrecedenceInExpression(NodeTraversal t, Node n, final int precedence) {
  ...
  void tryFoldLeftChildAdd(NodeTraversal t, Node n, Node left, Node right, Node parent) {
    ...
    if (NodeUtil.isLiteralValue(right) && left.getType() == Token.ADD && left.getChildCount() == 2) {
      ...
      if (lr.getType() != Token.STRING) return;
  ...
  void tryFoldBitAndOr(NodeTraversal t, Node n, Node left, Node right, Node parent) {
    ...
    if (left.getType() == Token.NUMBER && right.getType() == Token.NUMBER) {
      ...
      switch (n.getType()) {
        case Token.BITAND: result = lvalInt & rvalInt;
          break;
        case Token.BITOR: result = lvalInt | rvalInt;
          break;
        default: throw new Error("Unknown bitwise operator");
      }
      parent.replaceChild(n, Node.newNumber(result));
      t.getCompiler().reportCodeChange();
    }
   ...
   */
  @SuppressWarnings("fallthrough") void tryFoldComparison(NodeTraversal t, Node n, Node left, Node right, Node parent) {
    ...
    switch (left.getType()) {
      case Token.VOID: if (!NodeUtil.isLiteralValue(left.getFirstChild())) {
          return;
          ...
          switch (op) {
            case Token.EQ:
              ...
              result = equivalent;
              break;
            case Token.NE: result = !equivalent;
              break;
            case Token.SHEQ: result = undefinedRight;
              break;
            case Token.SHNE: result = !undefinedRight;
              break;
            case Token.LT: case Token.GT: case Token.LE: case Token.GE: result = false;
              break;
            default: return;
          }
        }
      ...
      case Token.NULL: if (undefinedRight) {
          result = (op == Token.EQ);
          break;
        }
        // fall through
      case Token.TRUE: case Token.FALSE: if (undefinedRight) {
        ...
        // fall through
      case Token.THIS: int tt = right.getType();
        if (tt != Token.THIS && tt != Token.TRUE && tt != Token.FALSE && tt != Token.NULL) return;
        switch (op) {
          case Token.EQ: result = left.getType() == right.getType();
            break;

          case Token.NE: result = left.getType() != right.getType();
            break;

          default: return;  // we only handle == and != here
      ...
      case Token.STRING: if (undefinedRight) {
        ...
        switch (op) {
          case Token.EQ: result = left.getString().equals(right.getString());
            break;

          case Token.NE: result = !left.getString().equals(right.getString());
            break;
      ...
      case Token.NUMBER: if (undefinedRight) {
        ...
        switch (op) {
          ...
          default: return;  // don't handle that op
            return;  // don't handle that op
        }
        break;

      case Token.NAME: if (rightLiteral) {
  ...
  void tryFoldGetProp(NodeTraversal t, Node n, Node left, Node right, Node parent) {
    if (right.getType() == Token.STRING && right.getString().equals("length")) {
      int knownLength = -1;
      switch (left.getType()) {
        case Token.ARRAYLIT: if (NodeUtil.mayHaveSideEffects(left)) {
  ...
  void tryFoldRegularExpressionConstructor( NodeTraversal t, Node n, Node parent) {
    ...
    Node pattern = constructor.getNext();  // e.g.  ^foobar$ Node flags = null != pattern ? pattern.getNext() : null;  // e.g. gi
    ...
    if (// is pattern folded pattern.getType() == Token.STRING
  ...
  private static Node makeForwardSlashBracketSafe(Node n) {
    ...
    for (int i = 0; i < s.length(); ++i) {
      switch (s.charAt(i)) {
        case '\\':  // skip over the next char after a '\\'. ++i;
          break;
        case '/':  // escape it if (null == sb) { sb = new StringBuilder(s.length() + 16); }
  ...
  void tryMinimizeCondition(NodeTraversal t, Node n, Node parent) {
    ...
    switch (n.getType()) {
      case Token.NOT: Node first = n.getFirstChild();
        switch (first.getType()) {
          case Token.NOT: {
              Node newRoot = first.removeFirstChild();
              parent.replaceChild(n, newRoot);
              n = newRoot; // continue from here. t.getCompiler().reportCodeChange();
    ...
    for (Node c = n.getFirstChild(); c != null; ) {
      Node next = c.getNext();  // c may be removed. tryMinimizeCondition(t, c, n);
      c = next;
    }
  }
