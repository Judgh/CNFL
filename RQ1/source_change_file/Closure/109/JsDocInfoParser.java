...
public final class JsDocInfoParser {
  ...
  private class ErrorReporterParser {
    void addParserWarning(String messageId, String messageArg, int lineno, int charno) {
      errorReporter.warning( SimpleErrorReporter.getMessage1(messageId, messageArg), getSourceName(), lineno, null, charno);
    }

    void addParserWarning(String messageId, int lineno, int charno) {
      errorReporter.warning(SimpleErrorReporter.getMessage0(messageId), getSourceName(), lineno, null, charno);
    }

    void addTypeWarning(String messageId, String messageArg, int lineno, int charno) {
      errorReporter.warning( "Bad type annotation. " + SimpleErrorReporter.getMessage1(messageId, messageArg), getSourceName(), lineno, null, charno);
    }

    void addTypeWarning(String messageId, int lineno, int charno) {
      errorReporter.warning( "Bad type annotation. " + SimpleErrorReporter.getMessage0(messageId), getSourceName(), lineno, null, charno);
    }
  }
  ...
  private static final Set<String> modifiesAnnotationKeywords = ImmutableSet.<String>of("this", "arguments");
  private static final Set<String> idGeneratorAnnotationKeywords = ImmutableSet.<String>of("unique", "consistent", "stable", "mapped");
  ...
  private enum State {
    ...
    SEARCHING_NEWLINE, NEXT_IS_ANNOTATION
    NEXT_IS_ANNOTATION
  }
  ...
  private JsDocToken parseAnnotation(JsDocToken token, List<ExtendedTypeInfo> extendedTypes) {
    ...
    if (annotation == null) {
      ...
      switch (annotation) {
        case NG_INJECT: if (jsdocBuilder.isNgInjectRecorded()) {
        ...
        case JAGGER_INJECT: if (jsdocBuilder.isJaggerInjectRecorded()) {
        ...
        case JAGGER_MODULE: if (jsdocBuilder.isJaggerModuleRecorded()) {
        ...
        case JAGGER_PROVIDE: if (jsdocBuilder.isJaggerProvideRecorded()) {
        ...
        case NOT_IMPLEMENTED: return eatUntilEOLIfNotAnnotation();
        ...
        case IDGENERATOR: token = parseIdGeneratorTag(next());
          return token;

        case WIZACTION: if (!jsdocBuilder.recordWizaction()) {
  ...
  private Node parseTypeExpression(JsDocToken token) {
    if (token == JsDocToken.QMARK) {
      ...
      token = next();
      if (token == JsDocToken.COMMA || token == JsDocToken.EQUALS || token == JsDocToken.RB || token == JsDocToken.RC || token == JsDocToken.RP || token == JsDocToken.PIPE || token == JsDocToken.GT || token == JsDocToken.EOC) {
