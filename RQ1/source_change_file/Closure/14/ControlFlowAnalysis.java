...
final class ControlFlowAnalysis implements Callback, CompilerPass {
  ...
  private void handleIf(Node node) {
    ...
    if (elseBlock == null) {
      createEdge(node, Branch.ON_FALSE, computeFollowNode(node, this)); // not taken branch } else {
  ...
  private void handleSwitch(Node node) {
    ...
    if (next != null) { // Has at least one CASE or EMPTY createEdge(node, Branch.UNCOND, next);
      ...
      } else { // No CASE, no DEFAULT createEdge(node, Branch.UNCOND, computeFollowNode(node, this));
      }
    }
  ...
  private void handleCase(Node node) {
    ...
    if (next != null) { // Found a CASE Preconditions.checkState(next.isCase());
      ...
      } else { // No DEFAULT found, go to the follow of the SWITCH. createEdge(node, Branch.ON_FALSE, computeFollowNode(node, this));
      }
    }
    connectToPossibleExceptionHandler(node, node.getFirstChild());
  }
  ...
  private static Node computeFollowNode( Node fromNode, Node node, ControlFlowAnalysis cfa) {
    ...
    switch (parent.getType()) {
      ...
      case Token.CASE: case Token.DEFAULT_CASE:
      ...
      case Token.TRY:
        ...
        if (parent.getFirstChild() == node) {
          ...
          } else { // and have no FINALLY. return computeFollowNode(fromNode, parent, cfa);
          }
        // CATCH block.
  ...
  private static class AstControlFlowGraph extends ControlFlowGraph<Node> {
    ...
    private AstControlFlowGraph(Node entry, Map<DiGraphNode<Node, Branch>, Integer> priorities, boolean edgeAnnotations) {
      super(entry, true /* node annotations */, edgeAnnotations);
