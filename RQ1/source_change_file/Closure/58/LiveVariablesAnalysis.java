...
class LiveVariablesAnalysis extends DataFlowAnalysis<Node, LiveVariablesAnalysis.LiveVariableLattice> {
  ...
  private void computeGenKill(Node n, BitSet gen, BitSet kill, boolean conditional) {
    ...
    switch (n.getType()) {
      case Token.SCRIPT: case Token.BLOCK: case Token.FUNCTION: return;

      case Token.WHILE: case Token.DO: case Token.IF: computeGenKill(NodeUtil.getConditionExpression(n), gen, kill, conditional);
        return;
      ...
      case Token.VAR: for (Node c = n.getFirstChild(); c != null; c = c.getNext()) {
          if (c.hasChildren()) {
      ...
      case Token.AND: case Token.OR: computeGenKill(n.getFirstChild(), gen, kill, conditional);
        // May short circuit.
      ...
      case Token.NAME: if (isArgumentsName(n)) {
      ...
      default: if (NodeUtil.isAssignmentOp(n) && NodeUtil.isName(n.getFirstChild())) {
          Node lhs = n.getFirstChild();
          if (!conditional) {
            addToSetIfLocal(lhs, kill);
          }
