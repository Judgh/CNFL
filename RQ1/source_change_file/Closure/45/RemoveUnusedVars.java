...
class RemoveUnusedVars implements CompilerPass, OptimizeCalls.CallGraphCompilerPass {
   ...
   */
  private final Multimap<Var, Node> inheritsCalls = ArrayListMultimap.create();
  ...
  private void traverseNode(Node n, Node parent, Scope scope) {
    ...
    switch (type) {
      ...
      case Token.ASSIGN: Assign maybeAssign = Assign.maybeCreateAssign(n);
        if (maybeAssign != null) {
      ...
      case Token.NAME: var = scope.getVar(n.getString());
        if (parent.isVar()) {
  ...
  private void removeUnreferencedVars() {
    ...
    for (Iterator<Var> it = maybeUnreferenced.iterator(); it.hasNext(); ) {
      ...
      } else if (toRemove.isVar() && nameNode.hasChildren() && NodeUtil.mayHaveSideEffects(nameNode.getFirstChild())) {
