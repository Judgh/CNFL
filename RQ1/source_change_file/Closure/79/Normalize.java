  ...
  static class NormalizeStatements implements Callback {
    ...
    @Override public void visit(NodeTraversal t, Node n, Node parent) {
      switch (n.getType()) {
        ...
        case Token.FUNCTION: normalizeFunctionDeclaration(n);
          break;

        case Token.NAME: case Token.STRING: case Token.GET: case Token.SET: if (!compiler.getLifeCycleStage().isNormalizedObfuscated()) {
      ...
      for (Node next, c = n.getFirstChild(); c != null; c = next) {
        ...
        switch (c.getType()) {
          case Token.LABEL: extractForInitializer(c, insertBefore, insertBeforeParent);
            break;
          case Token.FOR: if (NodeUtil.isForIn(c)) {
