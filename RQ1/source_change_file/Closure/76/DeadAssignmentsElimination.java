...
class DeadAssignmentsElimination extends AbstractPostOrderCallback implements CompilerPass, ScopedCallback {
  ...
  private static final Predicate<Node> matchRemovableAssigns = new Predicate<Node>() {
    @Override public boolean apply(Node n) {
      return (NodeUtil.isAssignmentOp(n) && n.getFirstChild().getType() == Token.NAME) || n.getType() == Token.INC || n.getType() == Token.DEC;
    }
  };
  ...
  private void tryRemoveDeadAssignments(NodeTraversal t, ControlFlowGraph<Node> cfg) {
    ...
    for (DiGraphNode<Node, Branch> cfgNode : nodes) {
      ...
      switch (n.getType()) {
        case Token.IF: case Token.WHILE: case Token.DO: tryRemoveAssignment(t, NodeUtil.getConditionExpression(n), state);
          continue;
        case Token.FOR: if (!NodeUtil.isForIn(n)) {
            tryRemoveAssignment( t, NodeUtil.getConditionExpression(n), state);
          }
          continue;
        case Token.SWITCH: case Token.CASE: case Token.RETURN: if (n.hasChildren()) {
  ...
  private boolean isVariableStillLiveWithinExpression( Node n, Node exprRoot, String variable) {
    while (n != exprRoot) {
      ...
      switch (n.getParent().getType()) {
        ...
        default: for(Node sibling = n.getNext(); sibling != null;
  ...
  private enum VariableLiveness {
    MAYBE_LIVE, // May be still live in the current expression tree. READ, // Known there is a read left of it. KILL, // Known there is a write before any read.
    READ, // Known there is a read left of it. KILL, // Known there is a write before any read.
    ...
    switch (n.getType()) {
      ...
      default:
        ...
        for (Node child = n.getFirstChild();
          ...
          if (!ControlFlowGraph.isEnteringNewCfgNode(child)) { // Not a FUNCTION VariableLiveness state = isVariableReadBeforeKill(child, variable);
