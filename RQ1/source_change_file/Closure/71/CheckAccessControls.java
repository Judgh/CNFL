...
class CheckAccessControls implements ScopedCallback, HotSwapCompilerPass {
  ...
  private boolean shouldEmitDeprecationWarning( NodeTraversal t, Node n, Node parent) {
    ...
    if (t.inGlobalScope()) {
      if (!((parent.getType() == Token.CALL && parent.getFirstChild() == n) || n.getType() == Token.NEW)) {
        return false;
      }
    }

    // We can always assign to a deprecated property, to keep it up to date.
    if (n.getType() == Token.GETPROP && n == parent.getFirstChild() && NodeUtil.isAssignmentOp(parent)) {
      return false;
    }
  ...
  private boolean canAccessDeprecatedTypes(NodeTraversal t) {
    ...
    return
      ...
      (getTypeDeprecationInfo(t.getScope().getTypeOfThis()) != null) ||
        // Case #3
      (scopeRootParent != null && scopeRootParent.getType() == Token.ASSIGN && getTypeDeprecationInfo( getClassOfMethod(scopeRoot, scopeRootParent)) != null);
  }
