...
final class NameAnalyzer implements CompilerPass {
    ...
    @Override public void remove() {
      ...
      switch (parent.getType()) {
        case Token.VAR: Preconditions.checkState(parent.hasOneChild());
          ...
          break;
        case Token.FUNCTION: replaceWithRhs(containingNode, parent);
          break;
        case Token.ASSIGN: if (containingNode.isExprResult()) {
    ...
    private void recordConsumers(NodeTraversal t, Node n, Node recordNode) {
      ...
      switch (parent.getType()) {
        case Token.ASSIGN: if (n == parent.getLastChild()) {
          ...
          break;
        case Token.NAME: NameInformation ns = createNameInformation(t, parent);
          recordDepScope(recordNode, ns);
          break;
        case Token.OR: recordConsumers(t, parent, recordNode);
          break;
        case Token.AND:
  ...
  private class FindReferences implements Callback {
    ...
    @Override public boolean shouldTraverse(NodeTraversal t, Node n, Node parent) {
      ...
      if (n.isFor()) {
        ...
        } else { // n.getChildCount() == 3 Node decl = n.getFirstChild();
  ...
  private NameInformation createNameInformation(NodeTraversal t, Node n) {
    ...
    switch (rootNameNode.getType()) {
      ...
      case Token.THIS: if (t.inGlobalScope()) {
          NameInformation nameInfo = new NameInformation();
          if (name.indexOf('.') == 0) {
            nameInfo.name = name.substring(1);  // strip leading "." } else {
            nameInfo.name = name;
          }
  ...
  private void replaceTopLevelExpressionWithRhs(Node parent, Node n) {
    // validate inputs
    switch (parent.getType()) {
      case Token.BLOCK: case Token.SCRIPT: case Token.FOR: case Token.LABEL: break;
      default: throw new IllegalArgumentException( "Unsupported parent node type in replaceWithRhs " + Token.name(parent.getType()));
    }

    switch (n.getType()) {
      case Token.EXPR_RESULT: case Token.FUNCTION: case Token.VAR: break;
      case Token.ASSIGN: Preconditions.checkArgument(parent.isFor(), "Unsupported assignment in replaceWithRhs. parent: %s", Token.name(parent.getType()));
        break;
      default: throw new IllegalArgumentException( "Unsupported node type in replaceWithRhs " + Token.name(n.getType()));
    }
  ...
  private boolean valueConsumedByParent(Node n, Node parent) {
    ...
    switch (parent.getType()) {
      case Token.NAME: case Token.RETURN: return true;
      case Token.AND: case Token.OR: case Token.HOOK: return parent.getFirstChild() == n;
      case Token.FOR: return parent.getFirstChild().getNext() == n;
      case Token.IF: case Token.WHILE: return parent.getFirstChild() == n;
      case Token.DO: return parent.getLastChild() == n;
