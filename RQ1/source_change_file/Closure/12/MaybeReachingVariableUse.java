...
class MaybeReachingVariableUse extends DataFlowAnalysis<Node, MaybeReachingVariableUse.ReachingUses> {
  ...
  private void computeMayUse( Node n, Node cfgNode, ReachingUses output, boolean conditional) {
    switch (n.getType()) {
      ...
      case Token.BLOCK: case Token.FUNCTION: return;

      case Token.NAME: addToUseIfLocal(n.getString(), cfgNode, output);
        return;

      case Token.WHILE: case Token.DO: case Token.IF: computeMayUse( NodeUtil.getConditionExpression(n), cfgNode, output, conditional);
        return;
      ...
      case Token.AND: case Token.OR: computeMayUse(n.getLastChild(), cfgNode, output, true);
      ...
      case Token.VAR: Node varName = n.getFirstChild();
      ...
      default: if (NodeUtil.isAssignmentOp(n) && n.getFirstChild().isName()) {
