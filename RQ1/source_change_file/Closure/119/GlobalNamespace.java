...
class GlobalNamespace implements StaticScope<JSType>, StaticSymbolTable<GlobalNamespace.Name, GlobalNamespace.Ref> {
  ...
  private class BuildGlobalNamespace implements NodeTraversal.Callback {
    ...
    public void collect(JSModule module, Scope scope, Node n) {
      ...
      switch (n.getType()) {
        case Token.GETTER_DEF: case Token.SETTER_DEF: case Token.STRING_KEY:
        case Token.SETTER_DEF: case Token.STRING_KEY:
        case Token.STRING_KEY:
          ...
          isSet = true;
          switch (n.getType()) {
            case Token.STRING_KEY: type = getValueType(n.getFirstChild());
              break;
            case Token.GETTER_DEF: type = Name.Type.GET;
              break;
            case Token.SETTER_DEF: type = Name.Type.SET;
              break;
            default: throw new IllegalStateException("unexpected:" + n);
          }
          break;
        case Token.NAME:
          ...
          if (parent != null) {
            switch (parent.getType()) {
              ...
              case Token.FUNCTION: Node gramps = parent.getParent();
                ...
                break;
              case Token.INC: case Token.DEC: isSet = true;
    ...
    Name.Type getValueType(Node n) {
      switch (n.getType()) {
        case Token.OBJECTLIT: return Name.Type.OBJECTLIT;
        case Token.FUNCTION: return Name.Type.FUNCTION;
        case Token.OR:
     ...
     */
    void handleSetFromGlobal(JSModule module, Scope scope, Node n, Node parent, String name, boolean isPropAssign, Name.Type type) {
    ...
    void handleGet(JSModule module, Scope scope, Node n, Node parent, String name) {
      ...
      if (parent != null) {
        switch (parent.getType()) {
          case Token.IF: case Token.INSTANCEOF: case Token.TYPEOF: case Token.VOID: case Token.NOT: case Token.BITNOT: case Token.POS: case Token.NEG: break;
          case Token.CALL: if (n == parent.getFirstChild()) {
              // It is a call target
            ...
            break;
          case Token.NEW: type = n == parent.getFirstChild() ? Ref.Type.DIRECT_GET : Ref.Type.ALIASING_GET;
            break;
          ...
          case Token.AND:
            ...
            break;
          case Token.HOOK: if (n != parent.getFirstChild()) {
            ...
            break;
          case Token.DELPROP: type = Ref.Type.DELETE_PROP;
            break;
          default: type = Ref.Type.ALIASING_GET;
            break;
        }
      }
    ...
    Ref.Type determineGetTypeForHookOrBooleanExpr( JSModule module, Scope scope, Node parent, String name) {
      ...
      for (Node anc : parent.getAncestors()) {
        switch (anc.getType()) {
          case Token.INSTANCEOF: case Token.EXPR_RESULT: case Token.VAR: case Token.IF: case Token.WHILE: case Token.FOR: case Token.TYPEOF: case Token.VOID: case Token.NOT: case Token.BITNOT: case Token.POS: case Token.NEG: return Ref.Type.DIRECT_GET;
          case Token.HOOK: if (anc.getFirstChild() == prev) {
              return Ref.Type.DIRECT_GET;
            }
            break;
          case Token.ASSIGN: if (!name.equals(anc.getFirstChild().getQualifiedName())) {
              return Ref.Type.ALIASING_GET;
            }
            break;
          case Token.NAME:  // a variable declaration if (!name.equals(anc.getString())) {
              return Ref.Type.ALIASING_GET;
            }
            break;
          case Token.CALL: if (anc.getFirstChild() != prev) {
              return Ref.Type.ALIASING_GET;
            }
            break;
          case Token.DELPROP: return Ref.Type.DELETE_PROP;
        }
        prev = anc;
      }
      return Ref.Type.ALIASING_GET;
  ...
  static class Name implements StaticSlot<JSType> {
    ...
    void addRef(Ref ref) {
      addRefInternal(ref);
      switch (ref.type) {
        case SET_FROM_GLOBAL: if (declaration == null) {
            declaration = ref;
        ...
        case SET_FROM_LOCAL: localSets++;
          break;
        case PROTOTYPE_GET: case DIRECT_GET: totalGets++;
          break;
        case ALIASING_GET: aliasingGets++;
          totalGets++;
          break;
        case CALL_GET: callGets++;
          totalGets++;
          break;
    ...
    private static JSDocInfo getDocInfoForDeclaration(Ref ref) {
      if (ref.node != null) {
        ...
        switch (refParent.getType()) {
          case Token.FUNCTION: case Token.ASSIGN: return refParent.getJSDocInfo();
          case Token.VAR: return ref.node == refParent.getFirstChild() ? refParent.getJSDocInfo() : ref.node.getJSDocInfo();
        }
      }
  ...
  static class Ref implements StaticReference<JSType> {
    ...
    enum Type {
      SET_FROM_GLOBAL, SET_FROM_LOCAL, PROTOTYPE_GET, ALIASING_GET,     // Prevents a name's properties from being collapsed DIRECT_GET,       // Prevents a name from being completely eliminated CALL_GET,         // Prevents a name from being collapsed if never set DELETE_PROP,      // Prevents a name from being collapsed at all.
      SET_FROM_LOCAL, PROTOTYPE_GET, ALIASING_GET,     // Prevents a name's properties from being collapsed DIRECT_GET,       // Prevents a name from being completely eliminated CALL_GET,         // Prevents a name from being collapsed if never set DELETE_PROP,      // Prevents a name from being collapsed at all.
      PROTOTYPE_GET, ALIASING_GET,     // Prevents a name's properties from being collapsed DIRECT_GET,       // Prevents a name from being completely eliminated CALL_GET,         // Prevents a name from being collapsed if never set DELETE_PROP,      // Prevents a name from being collapsed at all.
      ALIASING_GET,     // Prevents a name's properties from being collapsed DIRECT_GET,       // Prevents a name from being completely eliminated CALL_GET,         // Prevents a name from being collapsed if never set DELETE_PROP,      // Prevents a name from being collapsed at all.
      DIRECT_GET,       // Prevents a name from being completely eliminated CALL_GET,         // Prevents a name from being collapsed if never set DELETE_PROP,      // Prevents a name from being collapsed at all.
      CALL_GET,         // Prevents a name from being collapsed if never set DELETE_PROP,      // Prevents a name from being collapsed at all.
