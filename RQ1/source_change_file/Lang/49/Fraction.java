...
public final class Fraction extends Number implements Comparable {
    ...
    public static Fraction getFraction(double value) {
        ...
        int numer0 = 0;  // the pre-previous int denom0 = 1;  // the pre-previous int numer1 = 1;  // the previous int denom1 = 0;  // the previous int numer2 = 0;  // the current, setup in calculation int denom2 = 0;  // the current, setup in calculation int a1 = (int) value;
        int a2 = 0;
        double x1 = 1;
        double x2 = 0;
        double y1 = value - a1;
...
//            System.out.println(">>" + delta1 +" "+ delta2+" "+(delta1 > delta2)+" "+i+" "+denom2);
    ...
    public Fraction pow(int power) {
        if (power == 1) {
            ...
            if (power==Integer.MIN_VALUE) { // MIN_VALUE can't be negated. return this.invert().pow(2).pow(-(power/2));
            }
            return this.invert().pow(-power);
        } else {
            Fraction f = this.multiplyBy(this);
            if ((power % 2) == 0) { // if even... return f.pow(power/2);
            } else { // if odd... return f.pow(power/2).multiplyBy(this);
            }
        }
    }
    ...
    private static int greatestCommonDivisor(int u, int v) {
        ...
        // B1. [Find power of 2]
        int k=0;
        while ((u&1)==0 && (v&1)==0 && k<31) { // while u and v are both even... u/=2; v/=2; k++; // cast out twos.
        ...
        do {
            ...
            while ((t&1)==0) { // while t is even.. t/=2; // cast out twos
                t/=2; // cast out twos
            }
            // B5 [reset max(u,v)]
            if (t>0) {
