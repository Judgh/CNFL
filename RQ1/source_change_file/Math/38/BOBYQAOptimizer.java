...
public class BOBYQAOptimizer extends BaseAbstractMultivariateSimpleBoundsOptimizer<MultivariateFunction> implements MultivariateOptimizer {
    ...
    /** {@inheritDoc} */
    @Override protected RealPointValuePair doOptimize() {
    ...
    private double bobyqb(double[] lowerBound, double[] upperBound) {
        ...
        for(;;) switch (state) {
        case 20: {
            printState(20); // XXX if (trustRegionCenterInterpolationPointIndex != kbase) {
                int ih = 0;
                for (int j = 0; j < n; j++) {
                    for (int i = 0; i <= j; i++) {
                        if (i < j) {
        ...
        case 60: {
            printState(60); // XXX final ArrayRealVector gnew = new ArrayRealVector(n);
        ...
        case 90: {
            printState(90); // XXX if (dsq <= xoptsq * ONE_OVER_A_THOUSAND) {
        ...
        case 230: {
            printState(230); // XXX for (int k = 0; k < npt; k++) {
                double suma = ZERO;
                double sumb = ZERO;
                double sum = ZERO;
                for (int j = 0; j < n; j++) {
            ...
            if (ntrits == 0) {
                ...
                if (denom < cauchy && cauchy > ZERO) {
                    ...
                    cauchy = ZERO; // XXX Useful statement? state = 230; break;
                }
        ...
        case 360: {
            printState(360); // XXX for (int i = 0; i < n; i++) {
                // Computing MIN
                // Computing MAX
            ...
            for (int k = 0; k < npt; k++) {
                ...
                final double d2 = d1 * d1; // "d1" must be squared first to prevent test failures. vquad += HALF * modelSecondDerivativesParameters.getEntry(k) * d2;
            }
        ...
        case 680: {
            printState(680); // XXX if (rho > stoppingTrustRegionRadius) {
                delta = HALF * rho;
        ...
        case 720: {
            printState(720); // XXX if (fAtInterpolationPoints.getEntry(trustRegionCenterInterpolationPointIndex) <= fsave) {
                for (int i = 0; i < n; i++) {
                    // Computing MIN
                    // Computing MAX
                    final double d3 = lowerBound[i];
    ...
    private void prelim(double[] lowerBound, double[] upperBound) {
        ...
        do {
            ...
            final int numEval = getEvaluations(); // nfm + 1 fAtInterpolationPoints.setEntry(nfm, f);
    ...
    private double[] trsbox( double delta, ArrayRealVector gnew, ArrayRealVector xbdi, ArrayRealVector s, ArrayRealVector hs, ArrayRealVector hred ) {
        ...
        for(;;) {
            switch (state) {
        case 20: {
            printState(20); // XXX beta = ZERO;
        }
        case 30: {
            printState(30); // XXX stepsq = ZERO;
            for (int i = 0; i < n; i++) {
                if (xbdi.getEntry(i) != ZERO) {
                    s.setEntry(i, ZERO);
                } else if (beta == ZERO) {
        ...
        case 100: {
            printState(100); // XXX if (nact >= n - 1) {
                state = 190; break;
            }
            dredsq = ZERO;
            dredg = ZERO;
