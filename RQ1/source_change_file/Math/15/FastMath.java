...
public class FastMath {
    ...
    private static double doubleHighPart(double d) {
        ...
        xl = xl & MASK_30BITS; // Drop low order bits return Double.longBitsToDouble(xl);
    }
    ...
    public static double sinh(double x) {
      ...
      if (x > 0.25) {
          ...
          // Adjust for yb
          rb += yb*denomr;                        // numerator rb += -ya * denomb * denomr * denomr;   // denominator
    ...
    private static double exp(double x, double extra, double[] hiPrec) {
       ...
       full double precision (52 bits).  Since z < 2^-10, we will have 62 bits of precision when combined with the contant 1.  This will be
       ...
       expansion. tempA is exact since intPartA and intPartB only have 22 bits each. tempB will have 52 bits of precision.
       tempA is exact since intPartA and intPartB only have 22 bits each. tempB will have 52 bits of precision.
       ...
       important.  For accuracy add by increasing size.  tempA is exact and much larger than the others.  If there are extra bits specified from the
    ...
    private static double expm1(double x, double hiPrecOut[]) {
        if (x != x || x == 0.0) { // NaN or zero return x;
        }
    ...
    private static double log(final double x, final double[] hiPrec) {
        if (x==0) { // Handle special case of +0/-0 return Double.NEGATIVE_INFINITY;
        }
    ...
    public static double log1p(final double x) {
        ...
        if (x > 1e-6 || x < -1e-6) {
            ...
            if (Double.isInfinite(lores)) { // Don't allow this to be converted to NaN return lores;
            }
    ...
    b = b + d;
        ...
        if (xb != 0.0) {
            t = ((costA + costB) * (cosEpsA + cosEpsB) - (sintA + sintB) * (sinEpsA + sinEpsB)) * xb;  // approximate cosine*xb c = a + t;
            d = -(c - a - t);
            a = c;
            b = b + d;
        }
    ...
    err = (sina - cosa*est) + (sinb - cosb*est);
        ...
        double err = (sina - esta*cosaa - esta*cosab - estb*cosaa - estb*cosab)/cosa;  // Correction for division rounding err += sinb/cosa;                     // Change in est due to sinb err += -sina * cosb / cosa / cosa;    // Change in est due to cosb
        err += sinb/cosa;                     // Change in est due to sinb err += -sina * cosb / cosa / cosa;    // Change in est due to cosb
    ...
    private static double atan(double xa, double xb, boolean leftPlane) {
        ...
        if (xa == 0.0) { // Matches +/- 0.0; return correct sign return leftPlane ? copySign(Math.PI, xa) : xa;
        }
        ...
        if (xa > 1.633123935319537E16) { // Very large input return (negate ^ leftPlane) ? (-Math.PI * F_1_2) : (Math.PI * F_1_2);
        }
    ...
    public static double atan2(double y, double x) {
        ...
        if (y == 0) {
            ...
            if (invx == 0) { // X is infinite if (x > 0) {
                    return y; // return +/- 0.0 } else {
        ...
        // Compute ratio r = y/x
        final double r = y / x;
        if (Double.isInfinite(r)) { // bypass calculations that can create NaN return atan(r, 0, x < 0);
        }
        ...
        if (ra == 0) { // Fix up the sign so atan works correctly ra = copySign(0d, y);
        }
    ...
    public static double asin(double x) {
      ...
      if (x == 0.0) { // Matches +/- 0.0; return correct sign return x;
      }
      ...
      rb += (x - ra*ya - ra*yb - rb*ya - rb*yb) / y;  // Correct for rounding in division rb += -x * dx / y / y;  // Add in effect additional bits of sqrt.
    ...
    public static double acos(double x) {
      ...
      // Did r overflow?
      if (Double.isInfinite(r)) { // x is effectively zero return Math.PI/2; // so return the appropriate value
      ...
      rb += (y - ra*xa - ra*xb - rb*xa - rb*xb) / x;  // Correct for rounding in division rb += yb / x;  // Add in effect additional bits of sqrt.
    ...
    public static double cbrt(double x) {
      ...
      if (exponent == -1023) {
          ...
          subnormal = true;
          x *= 1.8014398509481984E16;  // 2^54 inbits = Double.doubleToLongBits(x);
    ...
    public static double toRadians(double x) {
        if (Double.isInfinite(x) || x == 0.0) { // Matches +/- 0.0; return correct sign return x;
        }
    ...
    public static double copySign(double magnitude, double sign){
        ...
        if ((m >= 0 && s >= 0) || (m < 0 && s < 0)) { // Sign is currently OK return magnitude;
        }
