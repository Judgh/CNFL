    ...
    private static double roundUnscaled(double unscaled, double sign, int roundingMethod) {
        ...
        case BigDecimal.ROUND_HALF_EVEN : {
            ...
            if (fraction > 0.5) {
                ...
                if (Math.floor(unscaled) / 2.0 == Math.floor(Math .floor(unscaled) / 2.0)) { // even unscaled = Math.floor(unscaled);
                } else { // odd unscaled = Math.ceil(unscaled);
                }
            }
            break;
        }
        ...
        case BigDecimal.ROUND_UP : unscaled = Math.ceil(nextAfter(unscaled,  Double.POSITIVE_INFINITY));
            break;
        default : throw new IllegalArgumentException("Invalid rounding method.");
        }
        return unscaled;
    }
