...
#         debug_result = dict(result)  # 显式存储结果
#         return debug_result
#
...
#     for key in original_order:  # 按原始顺序遍历
#         merged[key] = a * data1.get(key, 0) + b * data2.get(key, 0)
#     sorted_by_value_desc = dict(sorted(merged.items(), key=lambda x: x[1], reverse=True))
#     return sorted_by_value_desc


def process_files(file1, file2):
    """
    处理文件，执行预过滤、匹配、确定性归一化和融合。
    """

    # --- 1. 读取可疑度文件，并进行预过滤 (只保留 score > 0) ---
    sus_scores = {}
    original_order_map = {}
    ...
    with open(file1, 'r', errors='ignore') as f:
        for i, line in enumerate(f):
            if line.strip() and ',' in line:
                try:
                    key, value_str = line.rsplit(',', 1)
                    score = float(value_str.strip())

                    if score > 0:
                        original_key = key.strip().replace('/', '.')
                        sus_scores[original_key] = score
                        original_order_map[original_key] = i

                except ValueError:
                    continue

    if not sus_scores:
        print(f"  - No statements with suspiciousness > 0 found in {os.path.basename(file1)}.")
        return {}

    # core_stmt_ids 是我们确定的、唯一的分析对象集合
    core_stmt_ids = list(sus_scores.keys())
                ...
                if line.strip() and ',' in line:
                    try:
                        ...
                        if key in sus_scores:
                            nat_scores[key] = float(value_str.strip())
                    except ValueError:
                        continue

    # --- 3. 构建一个包含所有原始分数的 DataFrame ---
    # 这个DataFrame的行数和顺序现在是固定的
    df = pd.DataFrame({'id': core_stmt_ids})
    df['sus_score'] = df['id'].map(sus_scores)
    df['nat_score'] = df['id'].map(nat_scores)  # 缺失的值会自动成为 NaN

    # --- 4. 【关键】智能填充缺失值 ---
    if not df['nat_score'].isnull().all():
        median_nat_score = df['nat_score'].median()
        df['nat_score'].fillna(median_nat_score, inplace=True)
    else:
        df['nat_score'].fillna(0, inplace=True)

    # --- 5. 【关键】在完整的、对齐的数据集上进行确定性归一化 ---
    ...
    df['sus_norm'] = normalize(df['sus_score'])
    df['nat_norm'] = normalize(df['nat_score'])

    # --- 6. 融合 ---
    df['final_score'] = a * df['sus_norm'] + b * df['nat_norm']

    # --- 7. 排序 ---
    df['original_order'] = df['id'].map(original_order_map)
    df_sorted = df.sort_values(by=['final_score', 'original_order'], ascending=[False, True])

    # 8. 返回最终结果字典
    return dict(zip(df_sorted['id'], df_sorted['final_score']))
...
def result_analysis_big(multiply_path,topN_path,buggy_lines_dir,output_dir):
    ...
    for (project_id, number), group in grouped_results:
        ...
        if min_rank1 == -1 and min_rank2 != -1:
            min_rank1 = min_rank2
        # 计算差值（仅当两者都有有效值时）
    ...
    # 写入合并统计文件
    merged_output_file = os.path.join(output_dir, f"a*sus+b*natural/project_min_ranks_stmt({a},{b}).txt")
    # 确保目录存在（自动创建所有不存在的父目录）
    os.makedirs(os.path.dirname(merged_output_file), exist_ok=True)
    with open(merged_output_file, 'w') as f:
        f.write("Project,now,before,Difference\n")
        for stat in merged_project_stats:
            line = f"{stat['project_id']},{stat['now']},{stat['before']},{stat['diff']}\n"
            f.write(line)
...
if __name__ == "__main__":
    ...
    for pro in proList:
        for topN in topNList:
            ...
            multiply_path = f"/home/gwj/entrop/entropy-apr-replication/RQ2_final/Deal_With_Result/{model}/{Slice}/a*sus+b*natural/{pro}/top{topN}"
            # 合并文件
            merge_files_with_weights(topN_path, natural_path, multiply_path)#将错误定位的结果和熵进行相结合
            ...
            rank_file = f'/home/gwj/entrop/entropy-apr-replication/RQ2_final/Experimen_Result/{model}/noSlice/{pro}/top{topN}/a*sus+b*natural/project_min_ranks_stmt({a},{b}).txt'
            analysis_file = f'/home/gwj/entrop/entropy-apr-replication/RQ2_final/Experimen_Result/{model}/noSlice/{pro}/top{topN}/a*sus+b*natural/result_proj_stmt({a},{b}).txt'
